...
void subhier_clean(struct mosquitto_db *db, struct _mosquitto_subhier *subhier, bool clean_next)
{
	...
	while(subhier){
		next = clean_next ? subhier->next : NULL;
		leaf = subhier->subs;
		while(leaf){
			//by zjw.
			//delete subhier for context
		...
		subhier_clean(db, subhier->children, true);
...
{
	subhier_clean(db, db->subs.children, true);
